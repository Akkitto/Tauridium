...
class Patch0407Tests(unittest.TestCase):
  ...
  def test_opencode_has_browser_default_title_and_workspace_route(self) -> None:
    ...
    self.assertIn('"opencode" => Some("https://opencode.ai/workspace/{teamId}/go")', self.recipes)
    self.assertIn('"hasTeamId": team_url.is_some()', self.recipes)
  ...
  def test_custom_url_template_values_are_removed_when_service_is_deleted(self) -> None:
    ...
    self.assertIn('delete serviceDownloadSettings[s.id];', body)
    self.assertIn('serviceDownloadSettings,', body)
