...
```text
git tag -a v0.4.6 -m "Tauridium 0.4.6"
...
Runtime ZIPs are target-qualified using the binary's actual Rust compilation target, so artifacts from different native builds can coexist in the same `release/` directory. Common examples are `tauridium-0.4.6-run-win-x64.zip`, `tauridium-0.4.6-run-win-arm64.zip`, `tauridium-0.4.6-run-linux-x64.zip`, `tauridium-0.4.6-run-linux-arm64.zip`, and `tauridium-0.4.6-run-macos-arm64.zip`. Source and documentation ZIP names remain target-neutral.
